...
Validator.prototype.validate = function(values, presentOnly, cb) {
  ...
  function validate(validation, cb) {
    ...
    async.each( Object.keys(requirements.data), function (key, cb) {
      ...
      var err = false;
      if (!curValidation.type || !(curValidation.type.startsWith('pg_') ||
          (curValidation.type === 'bigserial'))) {
         // This is not Postgresql specific type, validate with Anchor.
         err = anchor(value).to(requirements.data, values);
      }
